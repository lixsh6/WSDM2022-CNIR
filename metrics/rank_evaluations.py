...
class rank_eval():
    ...
    def eval(self, y_true, y_pred, k = 10, label='PSCM'):
        res = {}
        target_k = [1,3,5,10]

        y_true = map(lambda t: score2cutoff(label, t), y_true)
        #print('y_true:',y_true)
        #print('y_pred:', y_pred)
        ...
        if label == 'HUMAN':
            ...
            self.rel_threshold = 1
        '''
        res['map'] = self.map(y_true, y_pred)
        res['err'] = err(y_true, y_pred) #res['err'] = err(y_true, y_pred, max_grade=1)
